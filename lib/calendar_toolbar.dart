...
import 'package:flutter/material.dart' as flutter_material; // Alias for Material package
...
const double _basePaddingMedium = 12.0; // Add 4px space to the right of the template selector
const double _basePaddingLarge = 0.0; // Added
const double _baseShrinkableSizedBoxWidth = 330.0; // Added for shrinkable SizedBox
...
class CalendarToolbar extends StatelessWidget {
  ...
  Widget _buildIconButton(_ToolbarAction action, double size, Color defaultColor) {
    return flutter_material.IconButton(
      icon: FaIcon(action.icon, size: size, color: action.color ?? defaultColor),
  ...
  Widget _buildOverflowMenu(List<_ToolbarAction> actions, double size, Color color) {
        ...
        return actions.map((action) {
          if (action.customWidget != null) {
            return flutter_material.PopupMenuItem<_ToolbarAction>(
              value: action,
              enabled: false, // Custom widgets in menu are usually not selectable themselves
  ...
  Widget build(BuildContext context) {
    ...
    final Color toolbarBackgroundColor = isDarkMode ? theme.colorScheme.surfaceContainerHighest : flutter_material.Colors.white;
    final Color iconColor = const Color(0xFF50a7d1); // Specific color for icons
    final Color textColor = theme.textTheme.bodyLarge?.color ?? (isDarkMode ? flutter_material.Colors.white : flutter_material.Colors.black);
      ...
      customWidget: flutter_material.Padding(
        ...
        child: flutter_material.ElevatedButton(
          ...
          style: flutter_material.ElevatedButton.styleFrom(
            ...
            shape: flutter_material.RoundedRectangleBorder(borderRadius: flutter_material.BorderRadius.circular(5 * scale)),
            padding: flutter_material.EdgeInsets.symmetric(horizontal: 11 * scale, vertical: 13 * scale),
            tapTargetSize: flutter_material.MaterialTapTargetSize.shrinkWrap,
            minimumSize: Size.zero,
          ),
          child: flutter_material.Text('TODAY',
            style: flutter_material.TextStyle(color: buttonTextColor, fontSize: _baseTodayFontSize * scale),
          ),
        ),
      ),
    );
    ...
    final String displayDateText = dateRangeCaption ?? _formatMonthYear(displayDate);
    ...
    final _ToolbarAction monthYearTextAction = _ToolbarAction(
      ...
      customWidget: flutter_material.Padding(
        padding: flutter_material.EdgeInsets.symmetric(horizontal: _basePaddingLarge * scale),
        child: flutter_material.Text(
          displayDateText,
          style: flutter_material.TextStyle(fontWeight: flutter_material.FontWeight.bold, color: textColor, fontSize: _baseFontSizeMonthYear * scale),
        ),
      ),
    );

    final _ToolbarAction? templateSelectorAction = templateSelector == null || !showTemplateSelector
        ? null
        : _ToolbarAction(
    ...
    final double scaledPaddingLarge = _basePaddingLarge * scale; // Added this line

    const double iconButtonWidth = 48.0;
    final double todayButtonWidth = _getTextWidth('TODAY', flutter_material.TextStyle(fontSize: _baseTodayFontSize * scale)) + (2 * 11 * scale) + (2 * 3.0); // text width + horizontal padding + horizontal padding from custom widget
    final double monthYearTextWidth = _getTextWidth(displayDateText, flutter_material.TextStyle(fontWeight: flutter_material.FontWeight.bold, fontSize: scaledFontSizeMonthYear)) + (scaledPaddingLarge * 2);
    const double configureTemplateWidth = iconButtonWidth;
    final double templateSelectorWidth = templateSelector == null ? 0.0 : 157.0 * scale; // Match the width defined in TemplateSelector
    ...
    final List<({_ToolbarAction action, double width})> leftAndCenterActions = [
      ...
      (action: _ToolbarAction( // Placeholder for SegmentedButton
        icon: Icons.view_agenda, tooltip: 'Change View', menuText: 'Change View', onPressed: null, customWidget: flutter_material.SizedBox(width: segmentedButtonWidth, child: flutter_material.SegmentedButton<CalendarView>(
          segments: <ButtonSegment<CalendarView>>[
            ButtonSegment<CalendarView>(
              value: CalendarView.month,
              label: FaIcon(FontAwesomeIcons.calendar, size: scaledIconSize),
            ),
            ButtonSegment<CalendarView>(
              value: CalendarView.week,
              label: FaIcon(FontAwesomeIcons.calendarWeek, size: scaledIconSize),
            ),
            ButtonSegment<CalendarView>(
              value: CalendarView.day,
              label: FaIcon(FontAwesomeIcons.calendarDay, size: scaledIconSize),
            ),
          ],
    ...
    final List<({_ToolbarAction action, double width})> rightAlignedToolbarActions = [
      if (templateSelectorAction != null) (action: templateSelectorAction, width: templateSelectorWidth + scaledPaddingMedium),
    ...
    final List<({_ToolbarAction action, double width})> combinedOrderedActions = [
      ...leftAndCenterActions.where((item) => item.action != monthYearTextAction), // Exclude monthYearTextAction for now
      (action: monthYearTextAction, width: monthYearTextWidth), // Add it explicitly for ordering
    ...
    return flutter_material.Container(
      ...
      decoration: flutter_material.BoxDecoration(
        ...
        border: flutter_material.Border(bottom: flutter_material.BorderSide(color: theme.dividerColor)),
      ),
      child: flutter_material.LayoutBuilder(
        builder: (context, constraints) {
          ...
          final double maxAvailableWidth = constraints.maxWidth - (2 * 3 * scale);
          ...
          for (final item in leftAndCenterActions) {
            ...
            if (currentLeftWidth + item.width <= maxAvailableWidth - overflowButtonSpace) {
          ...
          for (int i = rightAlignedToolbarActions.length - 1; i >= 0; i--) {
            ...
            if (currentRightWidth + item.width <= maxAvailableWidth - overflowButtonSpace) {
              currentRightWidth += item.width;
              visibleRightActions.insert(0, item); // Insert at beginning to maintain original order
            } else {
              overflowActions.insert(0, item); // Insert at beginning for overflow
            }
          }
          ...
          final double remainingWidthForCenter = maxAvailableWidth - currentLeftWidth - currentRightWidth - (needsOverflowButton ? overflowButtonSpace : 0);
          if (monthYearTextWidth > remainingWidthForCenter) {
            overflowActions.add((action: monthYearTextAction, width: monthYearTextWidth));
          }

          overflowActions.sort((a, b) {
            final indexA = combinedOrderedActions.indexWhere((element) => element.action == a.action);
            final indexB = combinedOrderedActions.indexWhere((element) => element.action == b.action);
          ...
          return flutter_material.Row(
            children: [
              flutter_material.SizedBox(width: 3 * scale), // Small space on the left

              // Render visible left-aligned actions
              for (final item in visibleLeftActions)
                if (item.action == todayAction) item.action.customWidget!
                else if (item.action.customWidget != null && item.action.customWidget is flutter_material.SizedBox && (item.action.customWidget as flutter_material.SizedBox).child is flutter_material.SegmentedButton)
              ...
              flutter_material.Expanded(
                child: flutter_material.Row(
                  ...
                  children: [
                    if (!overflowActions.any((item) => item.action == monthYearTextAction))
              ...
              for (final item in visibleRightActions)
                if (item.action == templateSelectorAction)
                  flutter_material.Padding(
                    padding: flutter_material.EdgeInsets.only(right: scaledPaddingMedium),
              ...
              if (needsOverflowButton)
                _buildOverflowMenu(overflowActions.map((item) => item.action).toList(), scaledIconSize, iconColor),

              flutter_material.SizedBox(width: 3 * scale), // Small space on the right
            ],
          );
        },
      ),
